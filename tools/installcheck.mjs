...
// somewhere far away.
//
// Ported from vorlaut, which has had it since 2026-08-24, after this repository
// produced the failure it exists to prevent. On 2026-08-25 the design package
// was released at 1.15.0 with .btn.sm defined for the first time, and the pin
// here was updated - but node_modules in the main checkout still held 1.14.1.
// What went red was e2e/app.spec.ts, "the caption spans the composer, both ends
// on the same edge", with `Expected: 0, Received: 2`. Two pixels, in a test
// about edge alignment, in code that was correct. It reads as a layout
// regression and gets investigated as one; the actual cause was that .btn.sm
// did not exist in the installed copy, so the button kept .btn's padding and
// the caption's negative margin no longer matched it.
//
